...
class FixedQlibToRdAgentConverter:
    ...
    def calculate_adjustment_factor(self, close_data, adjclose_data):
        """计算复权因子"""
        # 避免除零错误
        close_safe = np.where(close_data == 0, 1e-10, close_data)
        factor = adjclose_data / close_safe
        
        # 处理异常值
        factor = np.where(np.isnan(factor), 1.0, factor)
        factor = np.where(np.isinf(factor), 1.0, factor)
        factor = np.where(factor <= 0, 1.0, factor)
    ...
    def convert(self):
        """执行转换"""
        logger.info("开始修复版Qlib到RD Agent格式转换...")
        ...
        # 5. 复权因子统计
        print(f"\n=== 复权因子统计 ===")
        print(f"复权因子最小值: {df['$factor'].min():.6f}")
        print(f"复权因子最大值: {df['$factor'].max():.6f}")
        print(f"复权因子平均值: {df['$factor'].mean():.6f}")
        print(f"复权因子中位数: {df['$factor'].median():.6f}")
        print(f"复权因子标准差: {df['$factor'].std():.6f}")
...
def main():
    ...
    output_file = "/home/shenzi/RD-Agent/qlib_converted_daily_pv_fixed.h5"
    
    # 创建转换器
    converter = FixedQlibToRdAgentConverter(qlib_dir, output_file)
    
    # 执行转换
    df = converter.convert()
    
    print(f"\n✅ 修复版转换成功！文件保存在: {output_file}")
    print("✅ 复权因子已正确计算！")

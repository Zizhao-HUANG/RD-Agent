...
def check_and_list_free_ports(start_port=19900, max_ports=10) -> None:
    ...
    if is_occupied:
        ...
        logger.warning(
            f"Port 19900 is occupied, please replace it with an available port when running the `rdagent ui` command. Available ports: {free_ports}"
        )
    else:
        logger.info(f"Port 19900 is not occupied, you can run the `rdagent ui` command")
